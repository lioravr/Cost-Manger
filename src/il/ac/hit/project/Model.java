...
public class Model implements IModel {
    ...
    public Model() throws MyException {
        try {
            //trying to get the class of the driver.
                ...
                throw new MyException("problem with registering driver" +
                        " to the driver manager",e);
        }
    }
    /**
    ...
    public boolean login(Account user) throws MyException {
        try {
            ...
            ResultSet executeQueryTable = preparedStmt.executeQuery();
            //result set empty means that the details' login entered to function incorrect.
            if (!executeQueryTable.next()){
                throw new MyException("Sorry, your UserName/ Password was incorrect" +
                        " please try again");
            }
            return true;
    ...
    public int addCost(Cost item)throws MyException {
        try {
            try {
                /*
                at first, the function create a connection to the DB,
                then it creates a query to
                and insert the values it's gets to the PreparedStatement and execute it.
                 */
                login(account);
                String query = " insert into costs (UserName, Category," +
                        " Description, Sum, Currency,Date)"
                        + " values (?, ?, ?, ?, ?,?)";
                PreparedStatement preparedStmt = connection.prepareStatement(query);
            ...
            //fail to execute the query.
                ...
                throw new MyException("problem with add the new cost," +
                        " please sure you entered a valid values" +
                        " at the field and try again!", e);
     ...
     * @param resultSet The table that create from execute the sql query.
     ...
     */
    public Vector<Vector<Object>> buildTable(ResultSet resultSet) throws SQLException {
    ...
    public  Vector<Vector<Object>> report(LocalDate start, LocalDate end) throws MyException {
        ...
        try {
            try {
                ...
                login(account);
                String query = "SELECT Category, Description, Sum, Currency,Date FROM" +
                        " costs WHERE Date between ? AND ? And UserName = ?";
                PreparedStatement preparedStmt = connection.prepareStatement(query);
                ...
                table = buildTable(executeQueryTable);
            //fail to execute the query.
            } catch (SQLException e) {
                throw new MyException("problem with export the report," +
                        " please try again!", e);
            } finally {
                //close the connection to the DB.
                    connection.close();
    ...
    public  Vector<Vector<Object>> getListCost()throws MyException {
        String query = " SELECT Category ,Description," +
                " Sum, Currency, Date FROM costs WHERE UserName = ?";
        Vector<Vector<Object>> table;
        try {
            try {
            ...
            //fail to execute the query.
                ...
                throw new MyException("problem with export the costs," +
                        " please try again!");
            } finally {
                    //close the connection to the DB.
                    connection.close();
    ...
    public Vector<String> getListComboBox(String query, String label) throws MyException {
        ...
        try {
            try {
                 ...
                 */
                while (executeQueryTable.next()) {
                    category = executeQueryTable.getString(label);
                    vector.add(category);
                }
            //fail to execute the query.
            } catch (SQLException e) {
